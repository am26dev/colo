    ...
    <footer style={{ background: "var(--cream-2)", color: "var(--brown-dark)" }}>
      <div className="container" style={{ padding: "4rem 28px" }}>
        <div className="text-center">
          <p style={{ fontFamily: "var(--serif)", fontSize: "2.0rem", fontStyle: "italic", color: "var(--brown)" }} className="md:text-3xl">
            <EditableText contentKey="footer.tagline" />
          </p>
          <p style={{ marginTop: "0.75rem", fontSize: "0.9375rem", color: "var(--muted)" }}>
            <EditableText contentKey="footer.agradecimento" />
          </p>
        </div>

        <div style={{ margin: "3rem 0", height: "1px", background: "rgba(107, 63, 31, 0.15)" }} />
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: "2.5rem" }} className="md:grid-cols-4">
          ...
          <div>
            <div style={{ fontSize: "0.75rem", textTransform: "uppercase", letterSpacing: "0.2em", color: "var(--muted)" }}>
              <EditableText contentKey="footer.contactos.label" />
            </div>
            <ul style={{ marginTop: "1rem", display: "flex", flexDirection: "column", gap: "0.75rem", listStyle: "none", padding: 0, fontSize: "0.9375rem" }}>
              <li>
                <a
                  href={get("footer.contactos.whatsapp.url") || whatsappLink(config.whatsapp)}
              ...
              <li>
                <a
                  href={get("footer.contactos.instagram.url") || safeExternalUrl(config.instagram)}
                  style={{ display: "inline-flex", alignItems: "center", gap: "0.5rem", color: "inherit", textDecoration: "none" }}
                  className="hover:text-[var(--rose-deep)]"
              ...
              <li>
                <a
                  href={get("footer.contactos.email.url") || "mailto:ola@colo.ao"}
                  style={{ display: "inline-flex", alignItems: "center", gap: "0.5rem", color: "inherit", textDecoration: "none" }}
                  className="hover:text-[var(--rose-deep)]"
                >
        ...
        <div style={{ marginTop: "3rem", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "space-between", gap: "0.5rem", borderTop: "1px solid rgba(107, 63, 31, 0.15)", paddingTop: "1.5rem", fontSize: "0.8125rem", color: "var(--muted)" }} className="md:flex-row">
          <div>© {new Date().getFullYear()} <EditableText contentKey="footer.copyright" /></div>
          <div><EditableText contentKey="footer.assinatura" /></div>
        </div>
      </div>
    </footer>
  );
